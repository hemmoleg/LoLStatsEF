...
export class MyChampionAveragesComponent implements OnInit
{
  ...
  championAverages = new Map<string, ChampionAverage>(); 
  allChampsAverage = new ChampionAverage();
  ...
  calcAverages(matches?: DBMatch[]){
    ...
    this.matches.forEach(match => {
      ...
      if(this.myParticipantsAndInfos.has(myParticipant.championName)){
        this.myParticipantsAndInfos.get(myParticipant.championName).push(new ParticipantAndInfo(myParticipant, match.info));
      } else {
        this.myParticipantsAndInfos.set(myParticipant.championName, [new ParticipantAndInfo(myParticipant, match.info)]);
      }
    })

    this.allChampsAverage = new ChampionAverage();
    let totalWins = 0;
    ...
    this.myParticipantsAndInfos.forEach((value, key) => {
      ...
      value.forEach((dbParticipantAndInfo) => {
        dbParticipantAndInfo.p.win ? wins++ : loses++;
        kills += dbParticipantAndInfo.p.kills;
        deaths += dbParticipantAndInfo.p.deaths;
        assists += dbParticipantAndInfo.p.assists;
        damageDealtToChampions += dbParticipantAndInfo.p.totalDamageDealtToChampions;
        duration += dbParticipantAndInfo.i.gameDuration;

        totalWins += dbParticipantAndInfo.p.win ? 1 : 0;
      })

      let championAverage = new ChampionAverage();
